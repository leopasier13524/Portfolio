...
export const SplashIntro = forwardRef<HTMLDivElement, SplashIntroProps>(
  function SplashIntro({ onSkip }, ref) {
    return (
      <div
        ref={ref}
        data-splash-root
      ...
      >
        {onSkip ? (
          <button
            ...
            data-splash-skip
            onClick={onSkip}
            className="pointer-events-auto absolute right-[max(1.25rem,env(safe-area-inset-right))] top-[max(1.25rem,env(safe-area-inset-top))] z-30 text-[10px] uppercase tracking-[0.34em] text-white/55 transition-colors hover:text-white"
          >
            Skip
          </button>
        ) : null}
